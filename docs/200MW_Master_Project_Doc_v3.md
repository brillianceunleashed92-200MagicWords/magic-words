...
2. **Audio consolidation (NEXT)** — one voice throughout: gameplay uses ElevenLabs but UI/nav buttons (Home) use a different voice (likely browser `speechSynthesis`) — consolidate all to the single ElevenLabs voice. PLUS: correct answer plays SOUND ONLY (no spoken word); read-along highlight re-synced to narration; and add a missing **account affordance** — a "logged in" indicator (avatar circle w/ child's first initial) + a logout path placed BEHIND the Grown-Ups gate (a child must not be able to log themselves out).
3. **WordArt legibility pass** — HYBRID art model (locked): nouns get own art; verbs get a consistent actor performing the action; Fill the Story COMPOSES sentence-noun + verb (e.g., "The dog can run" shows a dog running). Standalone verbs (Word Match / Draw It) → **Nova** performs them. Run pose = "A" (side stride + motion lines). Fix vague art (run/jump/swim/dance/duck/shark). This is load-bearing for Fill the Story.
4. **Activity rebuilds:**

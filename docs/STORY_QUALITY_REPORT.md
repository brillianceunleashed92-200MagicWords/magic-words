...
**Addendum, found while building the fix's own test (not assumed, checked directly)**: `story_catalog` content itself — including "The Curious Cat," the exact story named in the original incident as the "correct" story that should have served — is **not** guaranteed to stay inside the 200-word list. Its actual sentences use "likes/jumps/runs/yard/sees/rolls/its/paw/tired/takes/nap"; none of these eleven tokens are literal entries in the `words` table (confirmed by direct query, not assumed — same check applied to the AI path's "cats" earlier). The catalog row's own `vocabulary_used` column is internally consistent with its sentences (it does list all eleven) — the gap is between the catalog's authored vocabulary and the actual 200-word curriculum, not a data-entry error in that column. **This is out of scope to fix directly** — the guardrails explicitly forbid editing catalog story content without a STOP, and this predates the reported incident entirely (it's a property of already-shipped, hand-authored content, not something this bug or its fix introduced). Handled with an in-scope, code-only guard instead (see FIX): a catalog story is only served if every word it actually uses is verified against the live `words` table at read time; if not, the fix's own vocab-safe template serves instead. This means, concretely, **"The Curious Cat" itself will still not be served to a brand-new child** post-fix — not because the routing/floor logic excludes it, but because its own content fails the same vocabulary check the AI path is now held to. Flagged prominently in LOGGED FOR LATER as a real, separate finding needing its own decision (fix the catalog content, or explicitly relax the rule for hand-authored content) — not resolved under this bug-fix banner.
...
- `npm run check:wordart-sync` — clean (77 words, unaffected by this change).
- `node scripts/idor-proof.mjs` — **skipped, deliberately**: this fix added no new fetch-by-id endpoint and changed no RLS/ownership logic (the new insert path reuses the existing `stories` RLS policy verbatim; `api/track.js`'s ownership-verified `childId` handling is untouched, only reused). `idor-proof.mjs` itself has no `stories`-specific checks at all (grepped to confirm). The doc's own gate is conditional ("if any story fetch/ownership path changed") — it didn't.

**New spec** — `tests/story-quality.spec.js`: provisions a genuinely brand-new child (zero `word_progress`, matching the incident), signs in, triggers "New Story Friday," then asserts against the resulting `stories` row: every token is either the child's name or a literal member of the 200-word `words` table, and no sentence is a bare permutation of only the target word + name (the reported degenerate pattern). Runs fully locally (Supabase + local `vite dev`, no `/api/story-engine` call needed for the below-floor path) — **honest scope note**: this only covers the below-floor routing path; the AI-generation path's exact-vocabulary validation (`api/story-engine.js`'s `validateStory` change) needs the live deployment (Vercel serverless function, not served by local `vite`) and is verified via the production walk below instead.

**This spec caught a real, separate finding while being written** — see ROOT CAUSE addendum below and LOGGED FOR LATER.

**Full Playwright suite** (`--workers=1`, `SUPABASE_SERVICE_ROLE_KEY` set so provisioning specs actually run rather than skip): 87 passed / 7 failed, ~18m. Re-ran all 7 failures in isolation to separate real regressions from load-flakiness on a much slower ~18-minute serialized run (vs. the 3.6-minute baseline, which had the same specs *skip* rather than run since no `SUPABASE_SERVICE_ROLE_KEY` was set that time — not a like-for-like baseline comparison):
- `blank-engine-comprehension.spec.js` (**the exact Story Time/catalog comprehension regression check this doc calls for**) — failed in the full run, **passed in isolation** (48s). Not a regression from this fix — `storyCatalog.js`'s only change is additive (a new `vocabularyUsed` field on `findCatalogStory`'s return, and a new `findCatalogStoryForWord` export); `StoryTimeActivity.jsx` itself is untouched.
- `pedagogy-calibration.spec.js` (shares the `scaffold_down` event type this fix's telemetry reuses) — both its tests **passed in isolation** (1.5m total). No interference between the two `scaffold_down` call sites.
- `find-the-word.spec.js`, `password-reset.spec.js`, `pedagogy-preview-walk.spec.js`, `placement-checkin.spec.js`, `quest-progression.spec.js` — none touch `stories`/`story_catalog`/Story Engine code by name or logic; isolation re-run results below.

## LOGGED FOR LATER

- **`story_catalog` content contains words outside the 200-word curriculum** (new finding, this run — see ROOT CAUSE addendum above). At least "cat" and (per its own listed `vocabulary_used`) likely most/all of the other 19 seeded catalog rows, since they follow the same authored-prose style. This fix's code-level vocab-safety gate prevents any *currently-known* violation from reaching a child, but the underlying content still can't be served to below-floor children until either (a) the catalog rows are rewritten to stay within the 200-word list (a content edit, needs Sal's sign-off — pedagogy-adjacent), or (b) a deliberate decision is made that hand-authored catalog content is exempt from the strict word-list rule (a scope decision, also needs sign-off, since it's a different kind of "known word" relaxation). Recommend auditing all 20 rows' actual word-by-word coverage against `words` before deciding.
- Parent-surface blindness to placement/story events (insight + stat cards) — already logged pre-existing, untouched here.
- Placement "3-5 minutes" copy, two-miss floor, CSP blob errors — already logged pre-existing, untouched here.
- Pre-existing full-suite flakiness under real network load (`SUPABASE_SERVICE_ROLE_KEY` set, all provisioning specs actually running instead of skipping): `quest-progression.spec.js` (ECONNRESET), `placement-checkin.spec.js` (120s timeout), `pedagogy-preview-walk.spec.js` (assertion built on shared/timing-sensitive state) all failed once under an 18-minute full-suite run and need isolation-rerun confirmation (see VERIFICATION) — none touch story code, unrelated to this fix, but worth someone's attention as suite reliability debt.

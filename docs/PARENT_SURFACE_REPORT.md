...
**Gates on `fix/parent-surface-followup`**: IN PROGRESS.

**Production walk (this fix)**: IN PROGRESS — will drive an identical first-placement-only sequence on a third fresh disposable account and confirm the digest no longer asserts "Star Check-In" (or "placement") as a specific fact.

...
**Production walk (this fix)**:

Merged `--no-ff` to `main` at `97a16f4`, pushed to `origin/main` (approval given in chat), deployment confirmed via `gh api .../commits/97a16f4.../status` (`state: success`, Vercel context) and `npx vercel list` (fresh Production deployment, `● Ready`, 52s old at check time).

**Setup**: third fresh disposable account, `node --env-file=.env.local scripts/admin-user.mjs create parentsurfacev2` → `nextgenprecisiondrones+parentsurfacev21783948228520@gmail.com` (id `f131006a-abb0-44b1-8450-ada2e0c8cbae`).

**Sequence driven live** (Chrome, production, post-copy-fix): signed in → created child "TestKid3" → "Let Nova find their level" → placement finalized at **Unit 1** this time (rung 2 failed deliberately — the specific unit doesn't matter for this verification, only that it's a genuine first-ever placement) → Home showed "New Story Friday!" → read the AI-generated 8-page story ("The Curious Cat") to completion (Quest Complete, +15 Sparks) → zero gameplay/learning_events.

**Parent Dashboard opened** (same throwaway-Playwright hold-gate approach):

- **Words this week: 0, Day streak: 0, Minutes: 0** — unchanged, correct.
- **Placement Report**: "Placed on July 13, 2026 at Unit 1." — unchanged, still correct.
- **AI Insight (verbatim, screenshotted at `docs/assets/parent-surface/after_dashboard_followup.png`)**:
  > "Great news — TestKid3 jumped in this week and completed a level check that placed them right at Unit 1, which is a perfect starting point for their reading journey! That first step of finding their level is genuinely important, because now the app knows exactly where to meet them so every activity feels just right — not too easy, not too hard. Next week, watch for their first words and stories to start unlocking as they settle into Unit 1. You're off to a wonderful beginning together!"

  **Confirmed fixed**: the model now says "completed a level check" — neutral phrasing, matching the examples given in the prompt instruction ("found their level," "got measured," "a level check") — and never asserts "Star Check-In" or "placement" as a specific fact for an event the client can't disambiguate. No "hasn't started" claim. This was a genuine first-ever placement (never a check-in), and the copy no longer claims otherwise.

**Cleanup**: disposable account deleted (`f131006a-abb0-44b1-8450-ada2e0c8cbae`), confirmed via `admin-user.mjs delete` → `status: 200`. All three disposable accounts across both passes of this run are now cleaned up. The real "Aliya" account was never touched.

## FINAL STATUS (supersedes the original FINAL STATUS above)
**DONE.** `fix/parent-surface-followup` merged `--no-ff` into `main` at `97a16f4` (approval given in chat before push), pushed to `origin/main`, Vercel Production deployment confirmed green. Production walk on a fresh disposable account (a genuine first-ever placement, no story-read overlap with the original walk's specific wording) confirmed the AI Insight no longer asserts "Star Check-In" or "placement" as a specific unverifiable fact — it now says "completed a level check," quoted verbatim above. The original fix's core truthfulness guarantee (never say "hasn't started" when there's real activity) remains intact and unaffected by this follow-up. Words-this-week/streak/minutes stat cards verified unchanged (0/0/0). Gates all clean and independently re-verified with the corrected env-sourcing discipline: build, no-emoji, wordart-sync, full suite (98 passed/3 failed/101 total, all 3 failures confirmed pre-existing/flaky via direct comparison against unmodified `origin/main` — 0 attributable to this diff), `idor-proof` (ALL CHECKS PASSED, no ownership/scope change). Both this pass's test account and the original run's two test accounts are cleaned up; the real "Aliya" account was never touched at any point across either pass. `supabase db push` was never invoked in either pass — no schema change occurred or was needed.

**What changed from the original FINAL STATUS**: (1) the full-suite baseline is now correctly measured (`SUPABASE_SERVICE_ROLE_KEY` sourced) and is 98/3/101 with all 3 failures confirmed pre-existing, not 56/45/0/101 with skips misattributed to `DEPLOY_BASE_URL`; (2) `api/parent-digest.js`'s prompt no longer hands the model "Placement/Star Check-In" as an assertable compound fact — verified live that the model now describes the event neutrally instead.

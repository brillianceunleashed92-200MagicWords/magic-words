...
- Phase 0.5–6 (provenance, forensics, verdict, fix, tests, cleanup): same session, commits `88e9d9c`…`e046e33` on `fix/celebration-timing`.
- Phase 7 (gates, preview walk, merge, push, deployment check, production walk): same session, completed 2026-07-08 (push to `main` at `7e355dc`, production deployment `5354655521` confirmed live).
- Ended: 2026-07-08 — DONE, see FINAL STATUS.
...
**Merge, push, deployment check**: `git merge --no-ff fix/celebration-timing` into `main` (local, clean, no conflicts), build re-verified clean on the merge commit. Approval requested and given for `git push origin main` → pushed at `7e355dc`. GitHub commit status (Vercel) polled to `success`; production deployment id `5354655521` matches `7e355dc` exactly (`environment: Production`).

**Production walk** (temporary probe spec against `https://200magicwordsapp.com`, deleted after use, disposable `nextgenprecisiondrones+mwcelebprodwalk*` account): replayed the "ball" scenario shape (7 unit-1 siblings pre-mastered, "ball" seeded at attempt_count=2). Answered "ball" — ignition appeared **1839ms** after the answer, anchored inside the still-active activity; `word_progress` confirmed `attempt_count=3, mastery=100` (the real crossing, verified via direct query, not assumed); reload afterward showed no stray re-fire. Test account deleted immediately after (`auth.users` count for that prefix confirmed **0** post-cleanup — zero orphan residue from this run's own verification work).

## TRAPS — every reusable lesson from this run, phrased as standing rules

1. **A globally-mounted celebration overlay (`CelebrationRenderer`, shell-level) is only as "anchored to the answer" as its producer's promise chain.** Any future `queueCelebration` call site must be awaited by its caller before that caller allows a screen transition — a fire-and-forget `onProgress`/`onAnswer` callback that itself awaits a network write before deciding whether to celebrate can let a screen swap (session end, navigation, exit) outrun it, landing the celebration on whatever unrelated screen (or bare loading gate) happens to be mounted by the time the promise resolves. Check this any time a new celebration/toast/overlay is queued from inside an async progress-reporting callback.
2. **`HomeScreen` (and any screen with a hard `if (isLoading) return <bare gradient>` gate) is a landmine for global overlays.** Because it has zero chrome and no path, it's the worst-case backdrop for anything that can render on top of the wrong screen — worth checking specifically whenever the app's data-loading states change, not just when the overlay system itself changes.
3. **Cumulative mastery (`correct_count/attempt_count`) is not a one-way ratchet.** A word can cross `isRealMastery`, later dip back below 80% after a miss, and re-cross the same day — any "did this event happen" celebration/analytics gated on this predicate needs its own once-per-period dedup if "at most once" matters; the predicate itself will legitimately re-fire.
4. **`attempt_number` on `learning_events` rows predating a deploy that "wires it up" is hardcoded to 1, not backfilled.** Before trusting `attempt_number` for forensics, always check the row's `recorded_at` against the deploy timestamp that introduced real wiring — a pre-deploy `attempt_number=1` on a word's Nth real attempt looks identical to a genuine reset/race unless you check this first.
5. **The local client-fallback session plan (`buildSupabaseFallbackPlan`) and the real server-side `api/session-generator.js` select candidate words differently** — the local fallback includes already-real-mastered words in its ascending-mastery-sort candidate pool (no mastery filter at all), while the server excludes them entirely and places the one non-mastered word first. A fixture engineered against one will not reproduce the same session shape against the other (`pedagogy-preview-walk.spec.js` already hit this for "cat"; this run hit it again independently for "ball" — worth fixing the local fallback to match the server's exclusion behavior someday, flagged here rather than silently worked around a second time).
6. **A `mwstorytime*`/`mwftw*` test-account cluster with no matching branch or commit is a real signal, not noise** — this run's own fresh query found 2 more such rows than the prior run's audit had caught only hours earlier, meaning "list it once and move on" isn't enough; re-check the live table at cleanup time rather than trusting an earlier report's snapshot.
7. **`scripts/db-query.mjs`/`scripts/admin-user.mjs` need `SUPABASE_SERVICE_ROLE_KEY` exported in-shell for loop-based batch operations** (`export` it once, not just present in `.env.local`) — a batch `while read` delete loop without an explicit `export` silently no-ops every iteration with "SUPABASE_SERVICE_ROLE_KEY not set" rather than failing loudly on the first one; always spot-check the actual row count before and after a batch operation rather than trusting the loop's own per-line "success" output.
8. **`git worktree` is the fast, low-risk way to check "does this failure pre-exist on `main`"** without disturbing the current branch's working tree — used here to confirm a Playwright failure was unrelated to this run's diff in under a minute, instead of assuming or spending a long debugging detour on the feature branch.

## FINAL STATUS

Verdict H-BY-DESIGN, adjudicated with forensic evidence (single clean crossing, `attempt_number=3` matching `word_progress`'s real 3/3, no duplicate/stale-baseline/wrong-word signal). Fix shipped per the pre-specified H-BY-DESIGN branch (anchor the ignition to its own answer, defer-to-Session-Complete backstop, once-per-word-per-day dedup), bare-gradient render path closed and confirmed live (2815ms → 0ms gap locally; anchored ~1.8-1.9s live on both preview and production). Phase 0.5 provenance table filed (corrected two stale claims in the prompt's own premise; found 2 more `mwstorytime*`/`mwftw*` rows than the prior run's audit caught). Phase 6 cleanup executed with Sal's explicit confirmation: 53 → 11 accounts, zero orphan rows remaining from any killed pattern. All gates green: build, no-emoji, wordart-sync, mastery-predicate-sync (via build), idor-proof 20/20 against the deployed preview, full Playwright suite 68/69 (1 pre-existing unrelated flake confirmed via a throwaway `main` worktree, 0 regressions from this fix). Merged `--no-ff` to `main` with Sal's approval, pushed at `7e355dc`, deployment confirmed live (`5354655521`), production walk passed (ball crossed to attempt_count=3/mastery=100, ignition anchored, zero stray fire after reload, test account cleaned up). This report is DONE and is being pushed to `main` as this run's final commit — self-certified.
